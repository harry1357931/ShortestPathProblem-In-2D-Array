/** ShortestPathProblem -- Using Memo Array  
 *  ShortestPathProblemWithMemo.cpp
 *     
 *  Fall, 2011
 *
...
int MinSum(int row, int col) {              // Recursive function
     ...
     if(col==0){                      // Base case 
         Memo[row][col]= air[row][col];
         return air[row][col];           
      }
      if(Memo[row][col]==0){
           if(row==0)     
                Memo[row][col]= air[row][col] + Min( MinSum(4,col-1), MinSum(row, col-1), MinSum(row+1, col-1) );
           else if(row==4)
                Memo[row][col]=  air[row][col] + Min( MinSum(row-1,col-1), MinSum(row, col-1), MinSum(0, col-1) );
           else
                Memo[row][col]= air[row][col] + Min( MinSum(row-1, col-1), MinSum(row, col-1), MinSum(row+1, col-1) );
      }
